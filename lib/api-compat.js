...
// `capability` entries route through hub's own capability router (the
// engine is resolved dynamically, from whichever configured engine's
// live /manifest declares that capability name).
// `engine` + `enginePath` entries (BB26091205) route STATICALLY to one
// named engine, bypassing the capability registry -- for an engine (e.g.
// Qpress) that doesn't declare capabilities via the manifest protocol the
// other engines share. `enginePath` may use `:param` placeholders filled
// from `params` (see `paramFromQuery` on the same route entry).
...
const ROUTES = [
  ...
  { method: 'GET', path: '/api/learning', capability: 'study.courses' },
  { method: 'GET', path: '/api/learning/groups', capability: 'study.groups.list' },
  { method: 'POST', path: '/api/learning/group', capability: 'study.group.save' },
  { method: 'POST', path: '/api/learning/group/archive', capability: 'study.group.archive' },
  { method: 'POST', path: '/api/learning/course/status', capability: 'study.course.status' },
  { method: 'POST', path: '/api/learning/module/meta', capability: 'study.module.meta' },
  { method: 'GET', path: '/api/learning/lesson', capability: 'study.lesson' },
  { method: 'GET', path: '/api/learning/asset', capability: 'study.asset' },
  { method: 'GET', path: '/api/learning/notes', capability: 'study.note.get' },
  { method: 'POST', path: '/api/learning/notes', capability: 'study.note.save' },
  { method: 'POST', path: '/api/learning/resume', capability: 'study.resume' },
  { method: 'POST', path: '/api/learning/progress', capability: 'study.progress' },
  { method: 'GET', path: '/api/learning/contributions', capability: 'study.contributions' },
  { method: 'GET', path: '/api/learning/campus', capability: 'study.campus' },
  { method: 'GET', path: '/api/learning/prime', capability: 'study.prime' },
  { method: 'GET', path: '/api/articles/list', capability: 'articles.list' },
  { method: 'GET', path: '/api/learning/manifest', capability: 'study.manifest' },
  { method: 'GET', path: '/api/learning/audio', capability: 'study.audio.get' },
  { method: 'POST', path: '/api/learning/narrate', capability: 'study.audio.generate' },
  { method: 'POST', path: '/api/learning/parse-md', capability: 'study.parseMd' },
  ...
  //
  // BB26091205 (12/18 Sep 2026): D-009 absorbed the generation engine
  // itself into QSpace Press (a native Rust rewrite, `q-space/press`,
  // NOT the `isconl/scope` repo). The three routes below that map 1:1
  // onto what Press now actually serves are re-pointed there directly
  // (`engine: 'press'`, bypassing the capability registry -- see the
  // dispatch-site comment in src/server.js for why a static target,
  // not a manifest re-declaration). A 4th, brand-new route is added for
  // `GET /archetypes/:id/schema` (D-009's own acceptance criteria: "iSconl
  // wizard renders against /archetypes/:id/schema with no local copy" --
  // there was no caller for this at all before now, since scope's
  // engine never exposed a schema endpoint separately from the archetype
  // list). The remaining 9 routes in this block (`generate.docs.*`
  // persistence/management, `writerBinder.*`) stay on `scope` -- Press
  // has no DB-backed persistence yet (`posts`/`publications` are still
  // Week 2 scope there, see that repo's own doc comments), so there is
  // nothing on Press to point them at without inventing a dead endpoint.
  // `writer.research_field`/`writer.full_draft` (below) were never
  // scope's to begin with -- they're spark's AI single-field-assist
  // capabilities (D-013's "AI drafts one field, never a whole doc") and
  // are untouched by this row.
  { method: 'GET', path: '/api/generate/archetypes', engine: 'press', enginePath: '/archetypes' },
  { method: 'POST', path: '/api/generate/preview', engine: 'press', enginePath: '/archetypes/preview' },
  { method: 'POST', path: '/api/generate', engine: 'press', enginePath: '/archetypes/generate' },
  {
    method: 'GET',
    path: '/api/generate/archetypes/schema',
    engine: 'press',
    enginePath: '/archetypes/:archetypeId/schema',
    paramFromQuery: { archetypeId: 'archetypeId' },
  },
  ...
  { method: 'GET', path: '/api/cycle-theme', capability: 'cycleTheme.get' },
  { method: 'POST', path: '/api/cycle-theme', capability: 'cycleTheme.set' },
  ...
  // BM26091401: circle.dia.regenerate's own path is parameterised
  // (/people/:id/regenerate-dia) -- reuses the existing paramFromQuery
  // mechanism (already proven for a POST route, see ops.service.restart
  // below) rather than inventing a new routing style for one capability.
  // Built so no future dossier backfill needs VM/SSH access -- see
  // BM26091401 in build.md for why this exists.
  { method: 'POST', path: '/api/circle/regenerate-dia', capability: 'circle.dia.regenerate', paramFromQuery: { id: 'id' } },
  // BM26091204 step 4: tags-management -- plain passthroughs, same shape
  // as circle.touch above, no path params to substitute.
  { method: 'GET', path: '/api/circle/tags', capability: 'circle.tags.list' },
  { method: 'POST', path: '/api/circle/tags/add', capability: 'circle.tags.add' },
  { method: 'POST', path: '/api/circle/tags/remove', capability: 'circle.tags.remove' },
  { method: 'POST', path: '/api/circle/tags/rename', capability: 'circle.tags.rename' },
  { method: 'POST', path: '/api/circle/tags/merge', capability: 'circle.tags.merge' },
  { method: 'POST', path: '/api/circle/tags/delete', capability: 'circle.tags.delete' },
  ...
  // BI26091022: news/breaking-updates engine Settings UI -- competitors.tsv
  // CRUD + freeform location/topic list CRUD, both pulse-owned
  // (lib/news.js), same wiring shape as notifications.* just above.
  { method: 'GET', path: '/api/news/competitors', capability: 'news.competitors.list' },
  { method: 'POST', path: '/api/news/competitors', capability: 'news.competitors.set' },
  { method: 'POST', path: '/api/news/competitors/delete', capability: 'news.competitors.delete' },
  { method: 'GET', path: '/api/news/topics', capability: 'news.topics.list' },
  { method: 'POST', path: '/api/news/topics', capability: 'news.topics.set' },
  { method: 'POST', path: '/api/news/topics/delete', capability: 'news.topics.delete' },
  { method: 'GET', path: '/api/news/terms', capability: 'news.terms.list' },
  { method: 'POST', path: '/api/news/sweep', capability: 'news.sweep' },
  ...
  // BL26082601: iSpark Library catalog + this tenant's selection --
  // vault-owned (library-sync.js), only meaningful when LIBRARY_MEMORY_DIR
  // is configured on the tenant's vault instance.
  { method: 'GET', path: '/api/library/catalog', capability: 'library.catalog.list' },
  { method: 'GET', path: '/api/library/selection', capability: 'library.selection.get' },
  { method: 'POST', path: '/api/library/selection', capability: 'library.selection.set' },
  ...
  // BN26090610: was `legacy: true` -- proxied to the retired legacy monolith,
  // which server.js's own generic router now 501s unconditionally (legacy
  // was deleted 2026-08-15). The webconsole's own Ventures card
  // (hub/web/static/app.js's finVentures/finAddVenture) has been calling
  // these two paths this whole time -- confirmed dead. pulse now exposes a
  // real ventures.list/.set/.delete surface (finance.js, backed by the
  // same live vault data /api/projects already reads correctly) --
  // routed for real below, plus the two new write paths the editable UI needs.
  { method: 'GET', path: '/api/ventures', capability: 'finance.ventures.list' },
  { method: 'POST', path: '/api/ventures/upsert', capability: 'finance.ventures.set' },
  { method: 'POST', path: '/api/ventures/delete', capability: 'finance.ventures.delete' },
  { method: 'POST', path: '/api/ventures/discover', capability: 'ventures.discovery.run' },
  { method: 'GET', path: '/api/possessions', capability: 'finance.possessions.list' },
  { method: 'POST', path: '/api/possessions/upsert', capability: 'finance.possessions.set' },
  { method: 'POST', path: '/api/possessions/delete', capability: 'finance.possessions.delete' },
  ...
  // BI26091904: multi-machine dashboard -- the backend (BI26091506) already
  // declares every one of these capabilities in ops's manifest; only the
  // hub-side proxy entries were missing. `machine` in the query becomes
  // `:id` in the manifest path template via the same paramFromQuery reshape
  // the single-machine routes above already use.
  { method: 'GET', path: '/api/ops/machines', capability: 'ops.machines.list' },
  { method: 'GET', path: '/api/ops/machine/status', capability: 'ops.machine.status', paramFromQuery: { id: 'machine' } },
  { method: 'GET', path: '/api/ops/machine/deploy-status', capability: 'ops.machine.deploy.status', paramFromQuery: { id: 'machine' } },
  { method: 'GET', path: '/api/ops/machine/metrics', capability: 'ops.machine.metrics.history', paramFromQuery: { id: 'machine' } },
  { method: 'GET', path: '/api/ops/machine/logs', capability: 'ops.machine.logs.tail', paramFromQuery: { id: 'machine', name: 'service' } },
  { method: 'POST', path: '/api/ops/machine/service/restart', capability: 'ops.machine.service.restart', paramFromQuery: { id: 'machine', name: 'service' } },
  { method: 'POST', path: '/api/ops/machine/service/start', capability: 'ops.machine.service.start', paramFromQuery: { id: 'machine', name: 'service' } },
  { method: 'POST', path: '/api/ops/machine/service/stop', capability: 'ops.machine.service.stop', paramFromQuery: { id: 'machine', name: 'service' } },
  { method: 'POST', path: '/api/ops/machine/service/destroy', capability: 'ops.machine.service.destroy', paramFromQuery: { id: 'machine', name: 'service' } },
