...
const ROUTES = [
  ...
  { method: 'POST', path: '/api/learning/narrate', legacy: true },
  ...
  // pulse's insights.get is a real capability but its "calendar" card is a
  // permanently-hardcoded placeholder (pulse/lib/rhythm.js's DEFAULT_INSIGHTS,
  // "On August 1, 1971, the Concert for Bangladesh..." -- readInsightsOverride,
  // the module's own extension point for this, is never wired to anything
  // real). Legacy's own /api/insights had the identical bug and already
  // fixed it (see server.js:12619's own comment, "stuck on 1 August because
  // this line hardcoded it") with a real onThisDay() layered over
  // history/onthisday.tsv -- proxying to that instead of pulse's stub until
  // that corpus (9.4MB, deliberately not in today's sync loop -- see
  // _handoff/migration-log.md) has a real home in vault.
  ...
  // /api/state: rebuilt natively in server.js (composes time.now/tasks.list/
  // ideas.list/vault.read + secrets.status) -- checked before this table is
  // ever consulted, so it's not listed here even though the route exists.
