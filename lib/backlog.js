...
async function buildBacklog({ force = false, request = githubApiRequest, token: tokenOverride } = {}) {
  ...
  // Group entries by project: any path matching work/dev/<...>/_next/backlog/<file>
  ...
  const projects = new Map(); // name -> { backlogFiles: Map<filename, path>, docFiles: [{name, path}] }
  ...
  for (const e of entries) {
    ...
    if (!projects.has(project)) projects.set(project, { backlogFiles: new Map(), docFiles: [] });
  ...
  for (const [project, p] of projects) {
    ...
    if (!rows.length && !p.docFiles.length) continue; // nothing to show for this project
    result.push({
      project,
      rows,
...
module.exports = { buildBacklog, parseRows, CATEGORY_FILES };
