...
function envOrUser(name) {
  // hasOwnProperty, not a truthy check -- see the matching note on get()
  // below. An explicit empty override (BWS_ACCESS_TOKEN: '', a test
  // disabling secrets) must not fall through to the Windows registry
  // fallback, which on a machine with a persisted bootstrap credential
  // (registry, per this function's own header comment) silently restores
  // the real token and defeats the override entirely -- confirmed live
  // 20 Aug, this was why scope's Jira tests still leaked a real token
  // after fixing get() alone.
  if (Object.prototype.hasOwnProperty.call(process.env, name)) return process.env[name].trim();
...
function get(name, fallback = '') {
  // hasOwnProperty, not a truthy check -- an intentionally empty override
  // (tests clearing a secret for isolation, e.g. JIRA_HOST: '') must win
  // over the Bitwarden cache below it, not fall through to it. Found live
  // 20 Aug: this exact truthy-check was why several engines' test suites
  // (this one included -- the RENDER_API_KEY env-leak test failure, see
  // done.md's earlier entries) leaked real Bitwarden-sourced secrets into
  // tests that explicitly tried to clear them.
...
function applyToEnv() {
  const hasEnv = (k) => Object.prototype.hasOwnProperty.call(process.env, k);
  let applied = 0;
  // Bare names first, so they take precedence over any prefixed alias.
  // hasOwnProperty, not a truthy check -- an explicit empty-string override
  // (test isolation, e.g. JIRA_HOST: '') must survive init() too, not just
  // get() -- found live 26 Aug 2026 in scope, a truthy check here silently
  // clobbered process.env.JIRA_HOST back to the real Bitwarden value before
  // get() ever ran, defeating get()'s own hasOwnProperty fix and causing a
  // test suite to hit the real Jira board (WSRU-106 incident). Same bug
  // confirmed present in every engine's copy of this file; fixed fleet-wide.
  for (const [k, v] of _cache) {
    if (!aliasFor(k) && !hasEnv(k) && v) { process.env[k] = v; applied++; }
  }
  for (const [k, v] of _cache) {
    const bare = aliasFor(k);
    if (!bare || !v) continue;
    if (!hasEnv(k)) { process.env[k] = v; applied++; }
    if (!hasEnv(bare) && !_cache.has(bare)) { process.env[bare] = v; applied++; }
  }
  return applied;
}
